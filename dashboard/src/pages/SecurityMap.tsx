...
// Using mock map without actual Google Maps API key (for demo)
// Replace with actual key if needed for production use
...
export default function SecurityMap() {
  ...
  useEffect(() => {
    ...
    // For demo purposes, show message if no API key is configured
    // In production, configure actual Google Maps API key
    ...
    try {
      ...
      if (mapContainer) {
        mapContainer.innerHTML = `
          ...
          ">
            <div>
              <h3>Security Map Ready</h3>
              <p>Configure Google Maps API key for full mapping features</p>
              <p style="font-size: 12px; margin-top: 10px;">
                Add your API key in SecurityMap.tsx line 32
              </p>
            </div>
          </div>
        `
  ...
  // Find nearest checkpoint to user location
  const findNearestCheckpoint = useCallback((location: { lat: number; lng: number }) => {
  ...
  }, [map, filteredCheckpoints, userLocation, findNearestCheckpoint])
